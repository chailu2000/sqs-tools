/**
 * Command: Sync Local to S3
 * Requirements: 13.1, 13.8, 13.10, 22.3
 */
...
export async function syncLocalToS3(
    ...
    const deleteMissing = await vscode.window.showQuickPick(
        ['Yes', 'No'],
        {
            placeHolder: 'Delete S3 objects that do not exist locally?',
        },
    );
    ...
    const result = await vscode.window.withProgress(
        {
            ...
            title: options.dryRun ? 'Calculating sync changes…' : `Syncing to S3: ${path.basename(options.localPath)}`,
            cancellable: true,
        },
        async (progress, token) => {
            ...
            return await syncService.syncLocalToS3(
                options,
                vsCodeToken,
                (progressUpdate) => {
                    processedCount++;
...
function displaySyncResult(result: SyncResult, dryRun: boolean): void {
    ...
    if (result.errors.length > 0) {
        ...
        vscode.window.showWarningMessage(
            ...
            `${result.uploaded} uploaded, ${result.skipped} skipped, ${result.deleted} deleted`,
        );
