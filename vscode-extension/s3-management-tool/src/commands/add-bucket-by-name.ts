...
export async function addBucketByName(
    ...
    const validation = await s3Service.validateBucketAccess(name);
    if (!validation.valid) {
        vscode.window.showErrorMessage(
            `Cannot access bucket "${name}": ${validation.error}\n` +
    ...
    vscode.window.showInformationMessage(`Bucket "${name}" added successfully.`);
}
